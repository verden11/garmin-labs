...
- SDK `WatchUi.Picker` (API 1.2.0), `PickerFactory`, `PickerDelegate`; `Menu2` (3.0). A three-column Picker (month, day, year) compiles under strict type checking for fr965, fenix7, fenix6pro, fr255s and venu2s (now `DaysToGo/source/settings/`). **Not yet run on a device or in the simulator's settings trigger.**
