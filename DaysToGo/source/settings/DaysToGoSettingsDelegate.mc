...
class DaysToGoSettingsDelegate extends WatchUi.Menu2InputDelegate {
    ...
    private function pushDatePicker() as Void {
        ...
        var days = numberColumn(DaysToGoConfig.FIRST_DAY_OF_MONTH, DaysToGoConfig.MAX_DAY_OF_MONTH, null);
        ...
        var picker = new WatchUi.Picker({
            :title => title,
            :pattern => monthFirst ? [months, days, years] : [days, months, years],
            :defaults => monthFirst ? [monthIndex, dayIndex, yearIndex] : [dayIndex, monthIndex, yearIndex]
        });
