...
class DaysToGoCountdown {
    ...
    // This year's occurrence unless its day is over. A timed event whose hour
    // has arrived stays "today" until midnight, same as a one-off event.
    private static function nextOccurrenceYear(event as DaysToGoEvent, now as DaysToGoLocalTime) as Number {
        ...
        return eventDay < today ? now.year + 1 : now.year;
    }
