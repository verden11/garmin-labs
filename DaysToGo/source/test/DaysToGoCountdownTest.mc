...
function timedEvents(logger as Test.Logger) as Boolean {
    ...
    Test.assertEqual(r.phase, DaysToGoConfig.PHASE_PAST);
    Test.assertEqual(r.days, 1);
    // An every-year timed event also stays TODAY from its hour until midnight, then rolls to next year.
    r = resolveAt(12, 25, 0, 18, 2026, 12, 25, 64800);
    Test.assertEqual(r.year, 2026);
    Test.assertEqual(r.phase, DaysToGoConfig.PHASE_TODAY);
    r = resolveAt(12, 25, 0, 18, 2026, 12, 25, 80000);
    Test.assertEqual(r.phase, DaysToGoConfig.PHASE_TODAY);
    r = resolveAt(12, 25, 0, 18, 2026, 12, 26, 0);
    Test.assertEqual(r.year, 2027);
    Test.assertEqual(r.phase, DaysToGoConfig.PHASE_UPCOMING);
...
// 29 Feb of a leap year is a real date when set for one year.
(:test)
function specificLeapDayIsValid(logger as Test.Logger) as Boolean {
    var r = resolveAt(2, 29, 2028, -1, 2026, 9, 26, 0);
    Test.assertEqual(r.phase, DaysToGoConfig.PHASE_UPCOMING);
    Test.assertEqual(r.day, 29);
    // Every year, on the leap day itself.
    r = resolveAt(2, 29, 0, -1, 2028, 2, 29, 0);
    Test.assertEqual(r.phase, DaysToGoConfig.PHASE_TODAY);
    return true;
}
