...
// Weekday is worked out on the calendar (a Moment overflows in 2038). 0 = Sunday.
(:test)
function weekdaysMatchKnownDates(logger as Test.Logger) as Boolean {
    Test.assertEqual(DaysToGoCalendar.weekday(1970, 1, 1), 4);    // Thursday
    Test.assertEqual(DaysToGoCalendar.weekday(2026, 12, 25), 5);  // Friday
    Test.assertEqual(DaysToGoCalendar.weekday(2028, 2, 29), 2);   // Tuesday
    Test.assertEqual(DaysToGoCalendar.weekday(2050, 9, 30), 5);   // Friday
    Test.assertEqual(DaysToGoCalendar.weekday(2060, 9, 30), 4);   // Thursday
    return true;
}
