...
class DaysToGoEvent {
    ...
    static function fromSettings(kind as Number, month as Number, day as Number, year as Number, hourSetting as Number) as DaysToGoEvent {
        if (kind == DaysToGoConfig.EVENT_NEW_YEAR) {
            return new DaysToGoEvent(DaysToGoConfig.NEW_YEAR_MONTH, DaysToGoConfig.NEW_YEAR_DAY, DaysToGoConfig.EVERY_YEAR, DaysToGoConfig.NO_HOUR);
        }
        if (kind == DaysToGoConfig.EVENT_CHRISTMAS) {
            return new DaysToGoEvent(DaysToGoConfig.CHRISTMAS_MONTH, DaysToGoConfig.CHRISTMAS_DAY, DaysToGoConfig.EVERY_YEAR, DaysToGoConfig.NO_HOUR);
        }
