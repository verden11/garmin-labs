...
Many deterministic unit tests, simulator workflows, few physical-watch checks. Commands: [`development.md`](development.md). Simulator passes are not device proof ([ADR-022](decisions.md#adr-022)/[023](decisions.md#adr-023)).
...
| `HeroSetStoreTest` | In-memory storage + controllable clock: XP ratchet (no farming, [ADR-002](decisions.md#adr-002)), rollover + streaks, learning state round-trip and malformed-as-fresh, String keys only ([ADR-022](decisions.md#adr-022)), diagnostics log cap, unknown exercise throws |
...
| `HeroSetSaveFeedbackTest`, `HeroSetExitMenuTest` | Toast tier order ([ADR-041](decisions.md#adr-041)); exit menus save through their own view ([ADR-036](decisions.md#adr-036)) |
| `HeroSetMenuTest` | `prepare` on real menu; run in both builds (store menu has no toggle, [ADR-033](decisions.md#adr-033)) |
...
| `HeroSetScreenFitTest` | `everyScreenFitsThisDisplay`: every screen in widest state; fails on text outside the circle, overlap, or missing rows ([ADR-034](decisions.md#adr-034)/[035](decisions.md#adr-035)). Run per product and per language |
