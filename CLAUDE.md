...
Garmin watch app (Forerunner 965 first, 67 round AMOLED + MIP watches supported, `docs/compatibility.md`) (Connect IQ, Monkey C): daily 100 push-ups, sit-ups, squats, auto rep counting (beta), manual correction, persistent XP/rank/streak. No GPS/distance. Goal: paid Connect IQ Store launch.

**`docs/` = source of truth; this file only orientation + house rules.** Resume order:

1. `docs/go-to-market.md` → **Status checkpoint**: open + blocked items.
...
- Products: 67 in both manifests, round AMOLED + MIP (ADR-034/035/037/038); only `fr965` tested on real watch · min API 3.4.0 (no 4.x-only calls outside `has` checks; `monkeyc` won't catch them) · one class per file, `HeroSet` prefix.
